#!/usr/bin/env python3
"""Build single ATT SDP project guide (DOCX + Markdown source).

Usage:
  python3 scripts/build_project_guide.py
  # Output: docs/ATT_SDP_Project_Guide.docx
"""
...
OUT_DIR = ROOT / "docs"
OUT_MD = OUT_DIR / "ATT_SDP_Project_Guide.md"
OUT_HTML = OUT_DIR / ".build_project_guide.html"
OUT_DOCX = OUT_DIR / "ATT_SDP_Project_Guide.docx"

MD = r"""# ATT Service Delivery Platform — Project Guide
...
def md_to_html(md: str) -> str:
    ...
    css = """
    body { font-family: Calibri, Arial, sans-serif; font-size: 11pt; line-height: 1.4; margin: 0.75in; color: #222; }
    h1 { font-size: 22pt; color: #0a2540; border-bottom: 2px solid #0095da; padding-bottom: 6px; }
    h2 { font-size: 14pt; color: #0095da; margin-top: 1.4em; border-bottom: 1px solid #ddd; }
    ...
    th { background: #0a2540; color: white; text-align: left; padding: 6px 8px; }
    td { border: 1px solid #ccc; padding: 5px 8px; vertical-align: top; }
    code { background: #f4f4f4; padding: 1px 4px; font-size: 9.5pt; }
    pre { background: #f4f4f4; padding: 10px; font-size: 9pt; overflow-x: auto; border: 1px solid #ddd; }
...
def main() -> None:
    ...
    if r.returncode == 0 and OUT_DOCX.exists():
        kb = OUT_DOCX.stat().st_size // 1024
        print(f"Wrote {OUT_DOCX} ({kb} KB)")
    else:
        print(f"DOCX failed: {r.stderr}")
        print(f"Use {OUT_MD} — open in Word or Google Docs")
